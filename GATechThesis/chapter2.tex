...
\noindent A shell pair ($M$, $N$) is significant if
\begin{align} 
\max\sqrt{\mu(M)\mu(N)\nu(M)\nu(N)}<\tau. 
\end{align} 

\noindent Where $\tau$ is the screening threshold.  This is known as Schwarz screening.
Moreover, additional sparsity can be achieved if the screening is applied to function pairs rather than entire shell pairs.
Then, a function pair ($i$, $j$) is significant if 
\begin{align} 
\sqrt{\mu(i)\mu(j)\nu(i)\nu(j)}<\tau.
\end{align} 
...
1, 5, and 10 benzenes spaced 3Å apart. Sparsity masks were obtained by evaluating $\sqrt{\mu(i)\mu(j)\nu(i)\nu(j)}<\tau$.}
